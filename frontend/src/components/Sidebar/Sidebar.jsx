...
async function getChannelProfile(authUserData) {
  ...
  const channelProfileResponse = await axios.get(
    `/api/v1/users/c/${encodeURIComponent(user.username)}`,
    AUTH_REQUEST_CONFIG
  )

  const channelProfile = channelProfileResponse?.data?.data

  return {
    ...DEFAULT_CHANNEL_PROFILE,
    ...channelProfile,
    _id: channelProfile?._id ?? user?._id ?? null,
  }
}
...
function Sidebar() {
  ...
  const [channelProfile, setChannelProfile] = useState(() => ({
    ...
    username: authUserData?.username || DEFAULT_CHANNEL_PROFILE.username,
  }))
  const [isLoadingChannelProfile, setIsLoadingChannelProfile] = useState(authStatus)

  useEffect(() => {
    let isMounted = true

    if (!authStatus) {
      setIsLoadingChannelProfile(false)
      return () => {
        isMounted = false
      }
    }
    ...
    const fetchChannelProfile = async () => {
      setIsLoadingChannelProfile(true)
      try {
        const profile = await getChannelProfile(authUserData)
        if (isMounted) {
          setChannelProfile((prevProfile) => ({
            ...prevProfile,
            ...profile,
          }))
        }
        ...
        if (isMounted) {
          setChannelProfile((prevProfile) => ({
            ...DEFAULT_CHANNEL_PROFILE,
            _id: prevProfile._id || null,
            username: prevProfile.username || DEFAULT_CHANNEL_PROFILE.username,
          }))
        }
      } finally {
        if (isMounted) {
  ...
  return (
    <div className="h-screen overflow-y-auto bg-[#121212] text-white">
      <div className="flex min-h-[calc(100vh-66px)] sm:min-h-[calc(100vh-82px)]">
        <aside className="group fixed inset-x-0 bottom-0 z-40 w-full shrink-0 border-t border-white bg-[#121212] px-2 py-2 sm:absolute sm:inset-y-0 sm:max-w-17.5 sm:border-t-0 sm:border-r sm:py-6 sm:hover:max-w-62.5 lg:sticky lg:max-w-62.5">
          <ul className="flex justify-around gap-y-2 sm:sticky sm:top-26.5 sm:min-h-[calc(100vh-130px)] sm:flex-col">
            <li className="">
              <button
              onClick={() => navigate("/")}
              ...
              className="flex flex-col items-center justify-center border-white py-1 focus:text-[#ae7aff] sm:w-full sm:flex-row sm:border sm:p-1.5 sm:group-hover:justify-start sm:group-hover:px-4 sm:hover:bg-[#ae7aff] sm:hover:text-black sm:focus:border-[#ae7aff] sm:focus:bg-[#ae7aff] sm:focus:text-black lg:justify-start lg:px-4">
                <span className="inline-block w-5 shrink-0 sm:group-hover:mr-4 lg:mr-4">
                  <svg style={{ width: "100%" }} viewBox="0 0 22 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20.7 11.5L18.7005 9.5L16.7 11.5M19 10C19 14.9706 14.9706 19 10 19C5.02944 19 1 14.9706 1 10C1 5.02944 5.02944 1 10 1C13.3019 1 16.1885 2.77814 17.7545 5.42909M10 5V10L13 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                  </svg>
                </span>
            ...
            <li className="">
              <button
              onClick={() => navigate("/channel/playlist")}
              className="flex flex-col items-center justify-center border-white py-1 focus:text-[#ae7aff] sm:w-full sm:flex-row sm:border sm:p-1.5 sm:group-hover:justify-start sm:group-hover:px-4 sm:hover:bg-[#ae7aff] sm:hover:text-black sm:focus:border-[#ae7aff] sm:focus:bg-[#ae7aff] sm:focus:text-black lg:justify-start lg:px-4">
                <span className="inline-block w-5 shrink-0 sm:group-hover:mr-4 lg:mr-4">
                  <svg style={{ width: "100%" }} viewBox="0 0 22 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <li className="hidden sm:block">
              <button
              onClick={() => navigate("/t/terms")}
              className="flex flex-col items-center justify-center border-white py-1 focus:text-[#ae7aff] sm:w-full sm:flex-row sm:border sm:p-1.5 sm:group-hover:justify-start sm:group-hover:px-4 sm:hover:bg-[#ae7aff] sm:hover:text-black sm:focus:border-[#ae7aff] sm:focus:bg-[#ae7aff] sm:focus:text-black lg:justify-start lg:px-4">
                <span className="inline-block w-5 shrink-0 sm:group-hover:mr-4 lg:mr-4">
                  <svg style={{ width: "100%" }} viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M11 14C12.6569 14 14 12.6569 14 11C14 9.34315 12.6569 8 11 8C9.34315 8 8 9.34315 8 11C8 12.6569 9.34315 14 11 14Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
